...
function parseOsv(value: unknown): OsvAdvisoryResult {
  ...
  for (const vulnerability of value.vulns) {
    ...
    for (const alias of aliases) if (typeof alias === "string" && /^CVE-\d{4}-\d+$/i.test(alias)) cves.add(alias.toUpperCase());
  }
  return { status: "known", advisoryCount, cveCount: cves.size };
}
